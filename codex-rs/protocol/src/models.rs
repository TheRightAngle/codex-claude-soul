...
// Compaction: wired natively via core/templates/compact/prompt.md
...
pub fn assemble_base_instructions(features: &PromptFeatures) -> String {
    ...
    // Compaction: injected only when /compact is invoked, not every turn.
    // Simplify: injected only when /simplify is invoked, not every turn.
    // Session titles: generated by the system, not a prompt section.
    // These section files exist at prompts/sections/ for use by the
